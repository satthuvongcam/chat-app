...
const User = ({ user }) => {
    ...
    const fetchFriendRequests = async () => {
      try {
        const response = await fetch(
          `https://chat-app-api-flmn.onrender.com/friend-requests/sent/${userId}`
        )
  ...
  useEffect(() => {
    const fetchFriends = async () => {
      try {
        const response = await fetch(
          `https://chat-app-api-flmn.onrender.com/friends/${userId}`
        )
  ...
  const handleSendFriendRequest = async (currentUserId, selectedUserId) => {
    try {
      const response = await fetch(
        'https://chat-app-api-flmn.onrender.com/friend-request',
        {
          method: 'POST',
          headers: {
